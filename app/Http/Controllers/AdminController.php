...
class AdminController extends Controller
{
    ...
    public function getUserList(Request $request) {
        ...
        $usersQuery = User::withTrashed()->where('role_id', 2)
                            ->with('subscription')
    ...
    public function getOpenWorldOLD03(Request $request){
    ...
    public function getOpenWorld(Request $request)
    {
        ...
        ->join('scheduling_posts', 'posts.id', '=', 'scheduling_posts.post_id')
        ->orderBy('posts.id', 'desc') // Order by Post ID DESC
        ->select('posts.*') // Important to avoid column conflict
        ->paginate(6);
   ...
   public function createUser(){
    ...
    public function viewUserDeatilsOLD(Request $request, $id)
    {
    ...
    public function viewUserDeatils(Request $request, $id)
    {
        ...
        $blockUsers = BlockUser::where(['user_id' => $id, 'block' => 1])
                                ...
                                ->get();
        // dd($blockUsers);
        return view('admin.user.view', compact('user','blockUsers'));
    }

    public function addLegacyAlbumByAdmin(Request $request)
    {
        // dd($request->all());
        DB::beginTransaction();
        ...
        try {
            ...
            LegacyAlbumPurchaseHistory::create([
                'user_id' => $user->id,
                'album_count' => $request->album_count,
                'package_name' => 'Admin Added',
                'amount' => 0,
                'date' => $today,
                'status' => 'success',
                'payment_id' => 'admin_' . time(),
                'is_added_by_admin' => 1
            ]);

            // ✅ total used albums
            $total_used_album_count = LegacyAlbum::where('user_id', $user->id)->count();

            // ✅ total purchased
            $total_purchase = LegacyAlbumPurchaseHistory::where('user_id', $user->id)
                                ->sum('album_count');

            // ✅ remaining
            $remaining = $total_purchase - $total_used_album_count;

            if ($remaining < 0) {
                $remaining = 0;
            }

            $user->remaining_lagecy_count = $remaining;
            $user->save();
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'Legacy Albums added successfully by admin',
                'data' => [
                    'remaining_lagecy_count' => $remaining
                ]
            ]);
    ...
    public function addTagsByAdmin(Request $request)
    {
        DB::beginTransaction();
        ...
        try {
            ...
            // ✅ Save purchase history (Admin Added)
            TagsPurchaseHistory::create([
                'user_id'            => $user->id,
            ...
            // ✅ Total tags already used
            $total_used_tag_count = FamilyTagId::where('created_user_id', $user->id)->count();

            // ✅ Total purchased tags
            $total_purchase = TagsPurchaseHistory::where('user_id', $user->id)->sum('tag_count');

            // ✅ Remaining tags
            $remaining_tag_count = $total_purchase - $total_used_tag_count;

            if ($remaining_tag_count < 0) {
                $remaining_tag_count = 0;
            }

            // ✅ Update user remaining tags
            $user->remaining_tag_count = $remaining_tag_count;
            $user->save();

            DB::commit();

            return response()->json([
                'message' => 'Tags added successfully by admin',
                'status'  => 'success',
                'data'    => [
    ...
    public function editFamoryTag($id){
        ...
        $users = $users = User::where('role_id',2)->where('deleted_at', null)->orderBy('id', 'desc')->get();
    ...
    public function storeSubscriptionSetting(Request $request){
        ...
        try {
            ...
            $save->title = $request->title;
            $save->subscription_type = $request->subscription_type;
            $save->save();
            DB::commit();
    ...
    public function getFamoryTagPost($id)
    {
        $tag_data = FamilyTagId::where('family_tag_id',$id)->first();
        $posts = Post::where('tag_id',$tag_data->id)->orderBy('id','DESC')->paginate(12);
        $tag_id = $tag_data->family_tag_id;
    ...
    public function deceasedReportsOLD()
    {
        $datas = [];
        $now = Carbon::now();
    ...
    public function deceasedReports()
    {
        $datas = [];
        $now = Carbon::now();
        $get_death_user = User::where('is_dead',1)->whereNull('deleted_at')->get();

        foreach ($get_death_user as $key => $user)
        {
            $datas[] =  [
                       'id' => $user->id??"",
    ...
    public function deceasedReportDeleteOLD(Request $request)
    {
        try {
            ...
            foreach ($request->deleteData as $data) 
            {
                 if (is_array($data) && isset($data['user_id'])) 
                 {
                    ...
                    if($user->is_dead == 1)
                    {
                       $user->is_dead = 0;
                       $user->save();
                       $checkDeathData =  DeathConfirmation::where('user_id',$data['user_id'])->get();
                       if(!empty($checkDeathData))
                       {
                           DeathConfirmation::where('user_id',$data['user_id'])->delete();
                       }
    ...
    public function customNotification(){
        ...
        $users = User::whereNull('deleted_at')
                      ->where('ban_user',0) // not ban user
                      ->where('role_id',2) 
                      ->where('is_dead',0) // not death user
                      ->get();

        return view('admin.customNotification',compact('users'));
    }

    public function sendCustomNotification_Old(Request $request)
    {
        $validator = Validator::make($request->all(), [
    ...
    public function sendCustomNotification(Request $request)
    {
        ...
        try {
            ...
            // 🔥 ASYNC JOB DISPATCH
            SendCustomNotificationJob::dispatch($sender, $userIds, $title, $message);

            return redirect()
                ->route('customNotification')
                ->with('success', 'Notification queued successfully. It will be sent shortly.');
    ...
    public function viewPostDetails(Request $request,$id){
        ...
        $post = Post::with('scheduling_post','user')->where('id', $id)->first();
    ...
    public function openWorldPostHiddenOLD03March(Request $request){
        try {
    ...
    {
        // Current Year (can be changed later via dropdown)
        $year = date('Y');
        ...
        */
        ...
        $tags = FamilyTagId::selectRaw('MONTH(created_at) as month, COUNT(*) as total')
            ->whereYear('created_at', $year)
            ->groupBy('month')
            ->pluck('total', 'month');
