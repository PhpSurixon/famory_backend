            ...
            <div class="card-info d-flex justify-content-between">
               <div class="card-header  d-flex gap-4">
                  ...
                  <div class="info-grp">
                     ...
                     <div class="d-flex gap-2">
                        <h5 class="text-dark">Phone No:- </h5>
                        <h5 class="fw-light">{{$user->phone ?? '-'}}</h5>
                     </div>
                     <div class="d-flex gap-2 align-items-center">
                        <h5 class="text-dark">Remaining Legacy Albums :-</h5>
                        <h5 class="fw-light">{{$user->remaining_lagecy_count ?? '0' }}</h5>
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        <h5 class="text-dark">Remaining Tags :-</h5>
                        <h5 class="fw-light">{{$user->remaining_tag_count ?? '0' }}</h5>
...
<!-- Legacy Album Modal -->
<div class="modal fade" id="legacyAlbumModal" tabindex="-1">
    <div class="modal-dialog">
        <form id="legacyAlbumForm" method="post">
            @csrf
            <input type="hidden" name="user_id" value="{{$user->id}}">

            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Add Legacy Albums</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>

                <div class="modal-body">
                    <label>Enter Album Count</label>
                    <select name="album_count" class="form-control">
    ...
    <div class="modal-dialog">
        <form id="tagForm" method="post">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <label>Enter Tag Count</label>
                    <select name="tag_count" class="form-control" required>
                        <option value="">Select Count</option>
                        <option value="1">1 Unit</option>
                        <option value="20">20 Pack</option>
                    </select>
...
<script>
    $('#legacyAlbumForm').submit(function(e) {

    e.preventDefault();
    var form = $(this);

    $.ajax({
        url: "{{ route('admin.addLegacyAlbum') }}",
        type: "POST",
        data: form.serialize(),
        success: function(response) {
            $('#legacyAlbumModal').modal('hide');
            Swal.fire({
                title: "Success",
                text: response.message,
                icon: "success"
